...
const TRANSACTION_TYPE_COLOR = {
  BUY: {
    bg: 'rgba(78, 186, 111, 0.1)',
    color: '#4EBA6F',
    icon: '↓',
  },
  SELL: {
    bg: 'rgba(194, 59, 59, 0.1)',
    color: '#C23B3B',
    icon: '↑',
  },
  EXCHANGE: {
...
export const DashboardPage: React.FC = () => {
  ...
  // Preferencia guardada desde Ajustes.
  // balanceVisible se puede cambiar localmente sin modificar la preferencia.
  ...
  useEffect(() => {
    async function loadDashboardData() {
      ...
      // Si un endpoint falla, el resto puede seguir cargando.
      ...
      if (
        ...
        setError(
          'No pudimos conectar con el backend. Verificá tu autenticación.'
        );
      }
  ...
  function exportToCSV() {
    ...
    csv += `Fecha de exportacion,${new Date().toLocaleString(
      'es-AR'
    )}\n\n`;

    if (wallet) {
      csv += '--- RESUMEN ---\n';
      csv += `Patrimonio Neto (ARS),${totalPatrimonioARS.toFixed(
        2
      )}\n`;
      csv += `Patrimonio Neto (USD),${totalPatrimonioUSD.toFixed(
        2
      )}\n\n`;
      ...
      balancesConValor.forEach((b) => {
        const name =
          SUPPORTED_CURRENCIES[b.currencyCode]?.name ||
          b.currencyCode;

        csv += `${b.currencyCode},"${name}",${b.amount},${b.valueInARS.toFixed(
          2
        )}\n`;
      });
    ...
    csv +=
      'ID,Fecha,Tipo,Moneda Origen,Moneda Destino,Monto Origen,Monto Destino,Estado\n';

    if (transactions.length > 0) {
      transactions.forEach((t) => {
        csv += `${t.id},${new Date(
          t.createdAt
        ).toLocaleDateString('es-AR')},${TRANSACTION_TYPE_LABEL[t.type]},${
          t.fromCurrency
        },${t.toCurrency || '-'},${t.fromAmount},${
          t.toAmount || '-'
  ...
  const totalPatrimonioARS = balancesConValor.reduce(
    (sum, b) => sum + b.valueInARS,
    0
  );

  const totalPatrimonioUSD =
    totalPatrimonioARS / CONVERSION_RATES.USD;
  ...
  const mainBalanceARS =
    wallet?.balances.find(
      (b) => b.currencyCode === 'ARS'
    )?.amount || '0';

  const mainBalanceARSNum = parseFloat(mainBalanceARS);

  let aiSuggestion =
    '¡Billetera vacía! Realizá una operación para comenzar a diversificar en distintas monedas.';

  if (mainBalanceARSNum > 500000) {
    aiSuggestion =
      'Tenés una cantidad considerable de Pesos Argentinos. El dólar cotiza estable esta semana; podrías considerar diversificar para resguardar tu capital.';
  } else if (mainBalanceARSNum > 0) {
    aiSuggestion =
      '¡Buen comienzo! Recordá monitorear nuestra sección de cotizaciones en tiempo real para aprovechar el mejor momento.';
  }
    ...
    <div className={styles.container}>
      ...
      >
        <div>
          ...
          <p className={styles.subtitle}>
            Este es el estado de tu portafolio financiero hoy
          </p>
        </div>
      ...
      <div className={styles.summaryGrid}>
        ...
        <div className={styles.card}>
          ...
          <span className={styles.cardSub}>
            De {wallet ? wallet.balances.length : 0} monedas
            soportadas
          </span>
        </div>
        ...
        >
          ...
          <div>
            ...
            <div className={styles.balancesGrid}>
              {wallet?.balances.map((b) => {
                const currency =
                  SUPPORTED_CURRENCIES[b.currencyCode];
                ...
                const valueInARS =
                  amountNum *
                  (CONVERSION_RATES[b.currencyCode] || 1);
          ...
          <div className={styles.chartCard}>
            ...
            <div className={styles.chartContainer}>
              {balanceVisible ? (
                ...
                >
                  Saldo oculto — el gráfico se vuelve a mostrar
                  al activar la visibilidad.
                </div>
              )}
            </div>
          </div>
