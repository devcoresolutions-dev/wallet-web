...
export const OperationsPage: React.FC = () => {
  ...
  const [successTxId, setSuccessTxId] =
    useState<string | null>(null);
  ...
  useEffect(() => {
    async function loadWallet() {
      try {
        ...
        console.error(
          'Error cargando billetera en operaciones:',
          err
        );
      }
    }
  ...
  // Recalcular simulación y tasa real cuando cambian
  // las monedas, el monto o el tipo de operación.
  useEffect(() => {
    ...
    async function fetchSimulation() {
      ...
      try {
        ...
        if (isSubscribed) {
          console.error(
            'Error obteniendo cotizaciones de la API:',
            err
          );

          setSimError(
            err?.message ||
              'No se pudo obtener la cotización actual (503 Service Unavailable).'
          );
  ...
  const currentFromBalance =
    wallet?.balances.find(
      (b) => b.currencyCode === fromCurrency
    );

  const availableBalanceStr =
    currentFromBalance?.amount || '0';

  const availableBalanceNum =
    parseFloat(availableBalanceStr);
  ...
  function handlePresetPercent(percent: number) {
    ...
    const calculated =
      availableBalanceNum * (percent / 100);
  ...
  const chartHistoricalData = [
    {
      day: 'Lun',
      rate: currentRate * 0.985,
    },
    {
      day: 'Mar',
      rate: currentRate * 0.992,
    },
    {
      day: 'Mié',
      rate: currentRate * 0.988,
    },
    {
      day: 'Jue',
      rate: currentRate * 0.996,
    },
    {
      day: 'Vie',
      rate: currentRate * 1.002,
    },
    {
      day: 'Sáb',
      rate: currentRate * 0.998,
    },
    {
      day: 'Hoy',
      rate: currentRate,
  ...
  async function handleConfirmExecution() {
    ...
    try {
      ...
      const res =
        await executeSimulatedTransaction({
    ...
    <div className={styles.container}>
      ...
      <div className={styles.tickerBar}>
        ...
        >
          <Activity size={15} />
          <span>
            SIMULADOR CON COTIZACIÓN EN VIVO
          </span>
        </div>
        ...
        {rateInfo?.rateAgeMinutes !== undefined && (
          ...
          >
            Edad de cotización:{' '}
            {rateInfo.rateAgeMinutes} min (
            {rateInfo.source || 'REAL_FEED'})
          </div>
        )}
      </div>
      ...
      <div className={styles.operationsGrid}>
        ...
        <div className={styles.simulatorCard}>
          ...
          <div className={styles.currencyBox}>
            <div
              className={styles.currencyBoxHeader}
            >
            ...
            <div
              className={styles.currencyInputRow}
            >
              <input
                type="text"
          ...
          >
            <div
              className={
                styles.currencyBoxHeader
              }
            >
              <span>
                Tú Recibes (Neto Estimado)
              </span>
        ...
        <div className={styles.infoColumn}>
          <div className={styles.breakdownCard}>
            ...
            <div className={styles.breakdownList}>
              ...
              <div
                className={`${styles.breakdownRow} ${styles.breakdownRowBold}`}
              >
                <span>
                  Total a Acreditar
                </span>
